param(
    [string]$ContentFile = 'itcm-manual.content.ps1',
    [string]$OutputPrefix = 'ITCM_User_Manual_Encoders'
)
...
function Copy-AssetsIfPresent([string]$assetsDir) {
    $source = 'C:\\Users\\ITD\\Desktop\\ITCM-IMAGES'
...
function Build-Pdf {
    ...
    function Draw-Bullets {
        ...
        foreach ($it in $items) {
            ...
            $lineH = 14
            Ensure-Space (($lines.Length * $lineH) + 6)
            $gfx.DrawString('•', $fontP, [PdfSharp.Drawing.XBrushes]::Black, $margin, $y)
            $first = $true
            foreach ($ln in $lines) {
                $x = $margin + 14
...
function Main {
    ...
    if ([string]::IsNullOrWhiteSpace($safePrefix)) { $safePrefix = 'ITCM_Manual' }
