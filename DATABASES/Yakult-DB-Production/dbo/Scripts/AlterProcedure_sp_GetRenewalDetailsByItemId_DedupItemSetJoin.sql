-- Fixes sp_GetRenewalDetailsByItemId crashing (Dapper "Sequence contains more than one
-- element") when an Item now legitimately belongs to more than one Set (e.g. renewed into
-- a new Set via the Renew Items feature). Run this against the real production database
-- (already applied to YIMS_PROD).

SET QUOTED_IDENTIFIER ON;
SET ANSI_NULLS ON;
GO
ALTER PROCEDURE [dbo].[sp_GetRenewalDetailsByItemId]
    @ItemId INT
AS
BEGIN
    SET NOCOUNT ON;
