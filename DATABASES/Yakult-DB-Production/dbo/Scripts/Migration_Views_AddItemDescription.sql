-- ==========================================================================
-- MIGRATION: Add ItemDescription to renewal report views
-- AFFECTED VIEWS:
--   dbo.vw_Report_RenewalsWithItems
--   dbo.vw_Report_RenewalGroupsLatest
-- NOTE: vw_InvoiceItems already exposes i.Description AS ItemDescription.
-- ==========================================================================

-- ── 1. vw_Report_RenewalsWithItems ─────────────────────────────────────────

CREATE OR ALTER VIEW [dbo].[vw_Report_RenewalsWithItems]
AS
SELECT
...
-- ── 2. vw_Report_RenewalGroupsLatest ───────────────────────────────────────

CREATE OR ALTER VIEW [dbo].[vw_Report_RenewalGroupsLatest]
AS
...
SELECT
    -- ── Grouping columns (anchored to latest set's date) ──────────────────
    YEAR  (ISNULL(s.DispatchDate, s.CreatedAt))                  AS [Year],
    MONTH (ISNULL(s.DispatchDate, s.CreatedAt))                  AS MonthNumber,
    DATENAME(MONTH, ISNULL(s.DispatchDate, s.CreatedAt))         AS MonthName,
