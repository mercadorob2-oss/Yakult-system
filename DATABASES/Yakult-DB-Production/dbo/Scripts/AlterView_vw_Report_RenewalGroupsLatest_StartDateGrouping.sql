-- ==========================================================================
-- MIGRATION: Renewals (Grouped) Report [Year]/MonthNumber/MonthName grouping
-- columns now anchor to the latest set's own StartDate (coverage start),
-- not its DispatchDate/CreatedAt (document/creation date).
--
-- WHY: see AlterView_vw_Report_RenewalsWithItems_StartDateGrouping.sql —
-- same reasoning, applied to the Grouped report's "latest set per chain"
-- view. Falls back to DispatchDate/CreatedAt only when StartDate is NULL.
--
...
ALTER VIEW [dbo].[vw_Report_RenewalGroupsLatest]
AS
