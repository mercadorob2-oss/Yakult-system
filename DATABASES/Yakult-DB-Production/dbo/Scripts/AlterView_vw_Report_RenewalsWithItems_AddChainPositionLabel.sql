-- ==========================================================================
-- MIGRATION: Replace the flat Renewals Report's "Old"/"Renewed" set banner
--            with a numbered ChainPositionLabel ("Renewal #1", "Renewal #2",
--            ...), matching the labeling scheme used on the Renewals Report
--            (Grouped). Computed via a recursive CTE over RenewalOfSetId,
--            same approach as the Grouped report's BuildFullChainSql.
...
SELECT
    ...
    s.SetType,
    s.RenewalOfSetId,
    'Renewal #' + CAST(ISNULL(rcl.ChainLevel, 0) + 1 AS VARCHAR(10)) AS ChainPositionLabel,
