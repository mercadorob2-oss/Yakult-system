-- ==========================================================================
-- MIGRATION: Renewals Report chain position label refinement.
-- Supersedes AlterView_vw_Report_RenewalsWithItems_AddChainPositionLabel.sql
-- (run that one first if it hasn't been applied yet; this one just changes
-- the label wording — the ChainLevel CTE itself is unchanged).
--
-- CHANGE: ChainLevel 0 (the never-renewed-from Set) is now labeled
-- "Original" instead of "Renewal #1". Every level above that is
-- "Renewal #N" using its own ChainLevel as N (so the first renewal is
-- "Renewal #1", the second is "Renewal #2", etc.).
