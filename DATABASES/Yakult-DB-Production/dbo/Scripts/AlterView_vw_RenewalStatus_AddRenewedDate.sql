-- Adds a RenewedDate column to vw_RenewalStatus (it was missing entirely, so the
-- "Renewed Date" column on the Renewals page was always blank), sourced from the most
-- recent non-archived dbo.Renewals row for any item still belonging to the Set.
