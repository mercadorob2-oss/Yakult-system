-- Migration: Extend sp_RepairPortal_CreateTicket with @RequestedByComId / @RequestedByBranchId.
-- When @RequestedByType = 'Department', the Company is now required (the same Department name can
-- exist under multiple Companies/Branches — see dbo.BranchDepartmentCompany) and, if a Branch is
-- also supplied, the Company/Branch/Department combination is validated against
-- dbo.BranchDepartmentCompany, mirroring the validation block in sp_Call_CreateTicket.
-- CREATE OR ALTER is idempotent — safe to re-run. This carries forward the full current body of
-- the proc from Migration_RepairPortal_CreateTicket_AddDateReceived.sql.
