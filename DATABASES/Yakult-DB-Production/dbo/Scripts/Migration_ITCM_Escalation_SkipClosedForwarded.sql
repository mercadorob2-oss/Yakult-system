-- Migration: Escalation worker skips Closed / Escalated / Forwarded to Repair.
-- Previously only Solved / Resolved (Temporary) were excluded, so Closed tickets
-- could be reassigned and flipped back to Escalated, Forwarded to Repair tickets
-- were escalated out from under the Repair workflow, and the supervisor branch
-- (which had no status check at all) churned already-Escalated assignees.
-- Mirrors the shared skip set used by the ITCM Server, standalone scheduler,
-- and desktop auto-escalation paths.
...
END
GO
PRINT 'sp_Call_ProcessEscalations updated (Closed/Escalated/Forwarded to Repair skipped).';
GO
