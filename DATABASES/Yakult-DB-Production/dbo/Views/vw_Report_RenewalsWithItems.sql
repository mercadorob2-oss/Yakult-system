-- ==========================================================================
-- VIEW: vw_Report_RenewalsWithItems
-- PURPOSE: Flat reporting view for the Renewals Report (item-level detail).
--          One row per SetItem within each renewal set.
--          Self-contained: does NOT depend on vw_Report_Renewals or
--          vw_RenewalStatus — all ExpiryStatus logic is inlined.
--          ROW_NUMBER() per SetCode gives the per-set item sequence (#).
-- INCLUDES: SetType IN ('Software/License', 'Service', 'Services')
-- ==========================================================================

CREATE VIEW [dbo].[vw_Report_RenewalsWithItems]
AS
