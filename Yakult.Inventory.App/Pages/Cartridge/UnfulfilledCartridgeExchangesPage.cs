...
{
    /// <summary>
    /// Page for tracking and managing unfulfilled cartridge exchanges.
    /// Displays returned but unfulfilled cartridges grouped by Set (same as Partially Fulfilled page),
    /// so multi-model exchange sessions are shown together.
    ///
    /// Business Rules:
    /// - Records are created when IssuedFullQty &lt; ReturnedEmptyQty
    /// - FIFO: Oldest pending records are fulfilled first when stock becomes available
    /// - Status: Pending (awaiting stock) or Fulfilled (completed)
    /// </summary>
    public partial class UnfulfilledCartridgeExchangesPage : UserControl
    {
        ...
        private List<UnfulfilledCartridgeExchangeDto> _allExchanges;
        private List<SessionGroup> _allSessions;
        private List<SessionGroup> _filteredSessions;

        private MaterialCard _cardPending;
        private MaterialCard _cardUnfulfilled;
        private MaterialCard _cardByModel;
        private Label _lblPendingCount;
        private Label _lblUnfulfilledQty;
        private Label _lblByModel;
        ...
        private HopeButton _btnFulfillSelected;
        ...
        // Constructor / init
        // -----------------------------------------------------------------------
        public UnfulfilledCartridgeExchangesPage()
        {
        ...
        private void BuildUI()
        {
            ...
            _layout = DefaultListPageTemplate.Create(
                "Unfulfilled Cartridge Exchanges",
            ...
            cmbStatusFilter.Items.AddRange(new object[] { "All", "Pending", "Fulfilled" });
            ...
            // Fulfill button
            _btnFulfillSelected = new HopeButton
            {
            ...
            UiFactory.ConfigureOutlineHopeButton(_btnFulfillSelected, Color.FromArgb(46, 204, 113), Color.FromArgb(230, 245, 230));
            _btnFulfillSelected.Click += BtnFulfillSelected_Click;
            _layout.ButtonLeftFlow.Controls.Add(_btnFulfillSelected);

            // Summary cards
            _cardPending    = UiFactory.CreateSummaryCard("Pending Records",       out _lblPendingCount,  Color.FromArgb(41, 128, 185));
            _cardUnfulfilled = UiFactory.CreateSummaryCard("Unfulfilled Cartridges", out _lblUnfulfilledQty, Color.FromArgb(231, 76, 60));

            _cardByModel = new MaterialCard
            {
                Size      = new Size(900, UiTheme.Sizes.SummaryCard.Height),
                BackColor = UiTheme.Colors.CardBack,
                Padding   = new Padding(12, 10, 12, 10),
                Margin    = new Padding(0, 0, 10, 0)
            };
            var lblByModelTitle = new Label
            {
                Text      = "By Model",
                AutoSize  = true,
            ...
            _cardByModel.Controls.Add(lblByModelTitle);
            _lblByModel = new Label
            {
                Location  = new Point(12, 32),
                Size      = new Size(_cardByModel.Width - 24, _cardByModel.Height - 44),
                Font      = new Font("Segoe UI", 9F),
            ...
            _cardByModel.Controls.Add(_lblByModel);
            _cardByModel.Resize += (s, e) =>
            {
                if (_lblByModel != null)
                    _lblByModel.Size = new Size(_cardByModel.Width - 24, _cardByModel.Height - 44);
            };

            _layout.SummaryFlow.Controls.Add(_cardPending);
            _layout.SummaryFlow.Controls.Add(_cardUnfulfilled);
            _layout.SummaryFlow.Controls.Add(_cardByModel);
        ...
        private void LoadData()
        {
            try
            {
                List<UnfulfilledCartridgeExchangeDto> data;
                string filter = cmbStatusFilter.SelectedItem?.ToString() ?? "Pending";

                switch (filter)
                {
                    case "Pending":   data = _repository.GetPurelyUnfulfilledExchanges(); break;
                    case "Fulfilled": data = _repository.GetFulfilledExchanges();         break;
                    default:          data = _repository.GetAllExchanges();               break;
                }

                _allExchanges = data ?? new List<UnfulfilledCartridgeExchangeDto>();
                _allSessions  = GroupIntoSessions(_allExchanges);
                ApplyFilter();
                UpdateSummary();
            }
            catch (Exception ex)
        ...
        private static List<SessionGroup> GroupIntoSessions(List<UnfulfilledCartridgeExchangeDto> rows)
        {
            ...
            var setMap     = new Dictionary<int, SessionGroup>();
            var sessionMap = new Dictionary<Guid, SessionGroup>();
            ...
            foreach (var row in rows)
            {
                if (row.SetId.HasValue && row.SetId.Value > 0)
                {
                    ...
                    {
                        grp = new SessionGroup { SetId = row.SetId, SessionId = row.SubmissionSessionId };
        ...
        private void UpdateSummary()
        {
            try
            {
                var (pendingCount, totalUnfulfilledQty) = _repository.GetPendingSummary();

                if (_lblPendingCount  != null) _lblPendingCount.Text  = pendingCount.ToString();
                if (_lblUnfulfilledQty != null) _lblUnfulfilledQty.Text = totalUnfulfilledQty.ToString();

                if (_lblByModel != null)
                {
                    var modelSummary = _repository.GetPendingSummaryByModel();
                    ...
                    _lblByModel.Text = models.Count == 0
                        ? "Model:"
            ...
            {
                if (_lblPendingCount   != null) _lblPendingCount.Text  = "0";
                if (_lblUnfulfilledQty != null) _lblUnfulfilledQty.Text = "0";
                if (_lblByModel        != null) _lblByModel.Text = "Model:";
            }
        }
        ...
        private static bool SessionMatchesSearch(SessionGroup session, string search, string filterBy) =>
            session.Rows.Any(r => RowMatchesSearch(r, search, filterBy));
        ...
        private static bool RowMatchesSearch(UnfulfilledCartridgeExchangeDto x, string search, string filterBy)
        {
            ...
            switch (filterBy)
            {
                ...
                default:
                    return Has(x.RequesterName) || Has(x.BranchName) || Has(x.DepartmentName)
                        || Has(x.CartridgeModel) || Has(x.ReqId.ToString());
            }
        }
        ...
        private void RebuildSessionBlocks(List<SessionGroup> sessions)
        {
            ...
            {
                _sessionsFlow.Controls.Add(new Label
                {
                    Text      = "No unfulfilled cartridge exchange sessions found.",
        ...
        private Panel CreateSessionBlock(SessionGroup session, int width)
        {
            ...
            dgv.Columns.Add(new DataGridViewTextBoxColumn { DataPropertyName = "Status",           HeaderText = "Status",   FillWeight = 12, MinimumWidth = 90 });
            ...
            dgv.CellFormatting += (s, e) =>
            {
                ...
                if (prop == "Status")
                {
                    ...
                    e.CellStyle.ForeColor = dto.Status == "Pending"
            ...
            dgv.SelectionChanged += (s, e) =>
            {
                ...
                {
                    _selectedDto = dto;
                    if (_btnFulfillSelected != null)
                        _btnFulfillSelected.Enabled = dto.Status == "Pending" && dto.UnfulfilledQty > 0;
                    ShowRemarks(dto.Remarks);
                }
                else
                {
                    _selectedDto = null;
                    if (_btnFulfillSelected != null) _btnFulfillSelected.Enabled = false;
                }
            };
        ...
        private void ShowRemarks(string remarks)
        {
            if (_lblRemarks == null) return;
            _lblRemarks.Text      = string.IsNullOrWhiteSpace(remarks) ? "No remarks" : $"Remarks: {remarks}";
        ...
        // Fulfill button
        // -----------------------------------------------------------------------
        private void BtnFulfillSelected_Click(object sender, EventArgs e)
        {
            ...
            // Gather all pending rows in the same Set (multi-model) or just this one
            List<UnfulfilledCartridgeExchangeDto> pendingRows;
            if (dto.SetId.HasValue && dto.SetId.Value > 0)
            {
                pendingRows = _allExchanges
                    ?.Where(x => x.SetId == dto.SetId && x.Status == "Pending" && x.UnfulfilledQty > 0)
                    .OrderBy(x => x.UnfulfilledId)
                    .ToList()
                    ?? new List<UnfulfilledCartridgeExchangeDto> { dto };
            }
            else
            {
                pendingRows = new List<UnfulfilledCartridgeExchangeDto> { dto };
            }
        ...
        private void ShowFulfillDialog(List<UnfulfilledCartridgeExchangeDto> pendingRows)
        {
            ...
            using (var dlg = new Form())
            {
                ...
                try
                {
                    ...
                    // Notify the requester of the fulfillment outcome
                    var anchor = states.FirstOrDefault(st => st.TotalToIssue > 0);
                    if (anchor != null)
                    {
                        int totalIssued    = states.Sum(st => st.TotalToIssue);
                        int totalRequested = states.Sum(st => st.Dto.UnfulfilledQty);
                        ...
                        if (totalIssued <= 0)
                        {
                            ...
                            notifMsg   = $"Your cartridge request (Req #{anchor.Dto.ReqId}) could not be fulfilled due to insufficient stock.";
                        }
                        else if (totalIssued < totalRequested)
                        {
                            ...
                            notifMsg   = $"Your cartridge request (Req #{anchor.Dto.ReqId}) was partially fulfilled: {totalIssued} of {totalRequested} issued.";
                        }
                        else
                        {
                            notifType  = NotificationType.RequestFulfilled;
                            notifTitle = "Request Fulfilled";
                            notifMsg   = $"Your cartridge request (Req #{anchor.Dto.ReqId}) has been fulfilled in full.";
                        }

                        try
                        {
                            var notifRepo = new NotificationRepository();
                            var (notifUserId, notifAuthId) = notifRepo.GetUserAndAuthIdForFulfillment(
                                anchor.Dto.EmpId, anchor.Dto.SubmissionSessionId);
