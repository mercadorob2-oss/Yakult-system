...
{
    internal class ExportRenewalPreviewDialog : Form
    {
        // ── Palette ──────────────────────────────────────────────────────────
        ...
        internal ExportRenewalPreviewDialog(List<RenewalDto> renewals, RenewalRepository repo)
        {
            _renewals = renewals;
            _repo = repo;
        ...
        private void InitializeComponentAsync()
        {
            ...
            // Fetch items for each renewal set
            var allItemIds = new HashSet<int>();
            foreach (var r in _renewals)
            {
                try
                {
                    var items = _repo.GetSetItemsForRenewal(r.SetId);
                    _setItems[r.SetId] = items ?? new List<SetItemRenewalDto>();
                    foreach (var item in _setItems[r.SetId])
                        allItemIds.Add(item.ItemId);
                }
                catch
                {
                    _setItems[r.SetId] = new List<SetItemRenewalDto>();
                }
            }
        ...
        private void BuildUi()
        {
            ...
            {
                Text      = $"Renewal Export Preview  —  {_renewals.Count} Renewal(s) selected",
            ...
            header.Controls.Add(new Label
            {
                Text      = "Review the selected renewals below, then click Export to PDF.",
        ...
        private string BuildPreviewHtml()
        {
            ...
            sb.Append(@"<!DOCTYPE html><html><head><meta charset='UTF-8'><title>Renewal Export Preview</title>
<style>
...
<div class='rpt-title'>
  <h1>Renewal Export Report</h1>
  <p>Generated: ");
            sb.Append($"{DateTime.Now:MMMM dd, yyyy  h:mm tt}  &nbsp;|&nbsp;  {_renewals.Count} Renewal(s)");
            sb.Append("</p></div>");

            // Sort by start date (or created date fallback), group by month
            ...
            var grouped = sorted
                .GroupBy(r =>
                {
                    var dt = r.StartDate ?? r.CreatedDate ?? DateTime.MinValue;
        ...
        private void OnExportToPdf(object sender, EventArgs e)
        {
            ...
            {
                Title      = "Save Renewal Export",
                Filter     = "PDF files (*.pdf)|*.pdf",
                FileName   = $"Renewal_Export_{DateTime.Now:yyyyMMdd_HHmmss}.pdf",
                DefaultExt = "pdf"
            })
            {
                ...
                try
                {
                    GenerateRenewalPdf(dlg.FileName);
                    Close();
        ...
        private void GenerateRenewalPdf(string outputPath)
        {
            // ── Layout constants ─────────────────────────────────────────────
            ...
            // ── Document & page state ────────────────────────────────────────
            var doc = new PdfDocument { Info = { Title = "Renewal Export Report", Author = "Yakult.Inventory.App" } };
            PdfPage   page = null;
            XGraphics gfx  = null;
            double    y    = 0;
            ...
            gfx.DrawString("Renewal Export Report", fTitle, XBrushes.White,
                new XRect(margin + 10, y, tableW - 10, hTitle * 0.62), XStringFormats.BottomLeft);
            string sub = $"Generated: {DateTime.Now:MMMM dd, yyyy  h:mm tt}   |   {_renewals.Count} Renewal(s)";
            ...
            foreach (var group in grouped)
            {
                ...
                gfx.DrawString($"({renewalCount} renewal{(renewalCount != 1 ? "s" : "")})", fMonthCnt, bGray,
            ...
            string totalText = $"{_renewals.Count} renewal(s)   \u00B7   ";
            ...
            string itemsText = $"{_renewals.Sum(r => r.ItemCount)}   \u00B7   ";
            ...
            gfx.DrawString($"{_renewals.Sum(r => r.TotalAmountDue):N2}", fFooter, bGray,
