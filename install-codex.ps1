...
function Download-File {
    ...
    & curl.exe -L --fail --silent --show-error $Uri -o $OutFile
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to download $Uri with curl.exe."
    }
}
    ...
    Invoke-WithInstallLock -LockPath $lockPath -Script {
        ...
        if (-not (Test-ReleaseIsComplete -ReleaseDir $releaseDir -ExpectedVersion $resolvedVersion -ExpectedTarget $target -Layout $installLayout)) {
            ...
            if ($installLayout -eq "Package") {
                Download-File -Uri $checksumMetadata.Url -OutFile $checksumPath
            ...
            Download-File -Uri $packageMetadata.Url -OutFile $archivePath
