# Yakult Inventory System — Codex Instructions
...
```
For csproj conflicts during pull: keep the local fixed version (`git checkout --ours`). For AGENTS.md conflicts: keep whichever version is more up to date.
...
## API Email Silently Dead — vw_Call_TicketList Missing Columns (TCK-000069)

**Symptom:** Mobile dry run completes end-to-end (HTTP 200s on create, field visits, photo upload, status changes) but desktop Ticket Email Log shows "No email log entries were found for this ticket." Desktop-triggered mail works fine.

**Root cause:** `GetTicket()` in `call-tickets.ashx` / `call-ticket-action.ashx` selects `AssignedTo, ProvidedSolution, DeptId, LastReminderSentAt` from `dbo.vw_Call_TicketList`, but the view never had those columns (in DEV, PROD, or the repo view definition). The `SqlException` throws inside the fire-and-forget `ThreadPool.QueueUserWorkItem` email sender whose `catch {}` swallows everything, so there is no email, no log row, and no error anywhere. Any `CallEmailLog` gap with zero rows (not even `Skipped`) for API-driven tickets points here.

**Fix (both applied 2026-09-16):**
- Handlers now check the four columns via `COL_LENGTH` (`ColExists` helper next to `ObjExists`) and fall back to the base-table query when absent. Always keep this guard when touching `GetTicket`.
- Migration `Migration_ITCM_TicketListView_EmailColumns.sql` adds the four columns to the view (durably deployed to DEV + PROD). The canonical definition in `DATABASES\Yakult-DB-Production\dbo\Views\vw_Call_TicketList.sql` still needs the same four columns added when it is next edited, or the view will regress on redeploy.
- Verification: run the handler's exact view query for the ticket in PROD/DEV (must return a row, not "Invalid column name"); a fresh mobile status change must then write a `CallEmailLog` row (`Sent`, or `Skipped`/`Failed` with a reason, never silent zero).

**Also note:** 723 `Failed` Reminder rows in PROD (`SMTP password decrypt failed`) are a separate scheduler/DPAPI issue, not this one.
