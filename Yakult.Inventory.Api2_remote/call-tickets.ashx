<%@ WebHandler Language="C#" Class="CallTicketsHandler" %>
using System;
...
public class CallTicketsHandler : IHttpHandler {
    ...
    void HandleGet(HttpContext c, CallTicketApiUser actor) {
        var status=(c.Request.QueryString["status"]??"").Trim();
        var scope=(c.Request.QueryString["scope"]??"").Trim().ToLowerInvariant();
        ...
        try {
            using(var con=new SqlConnection(Cs())){
                ...
                using(var cmd=new SqlCommand()){
                    ...
                    var finalStatuses = "('Solved','Resolved (Temporary)','Closed')";
                    if(scope=="inbox") {
                        sql+=" AND v.Status NOT IN "+finalStatuses+" AND v.AssignedToEmpId IS NULL";
                    }
                    else if(scope=="mine") {
                        if(actor==null || !actor.EmployeeId.HasValue || actor.EmployeeId.Value<=0) {
                            sql+=" AND 1=0";
                        } else {
                            sql+=" AND v.Status NOT IN "+finalStatuses+" AND v.AssignedToEmpId=@ActorEmployeeId";
                            cmd.Parameters.Add("@ActorEmployeeId",SqlDbType.Int).Value=actor.EmployeeId.Value;
                        }
                    }
                    else if(scope=="resolved") {
                        sql+=" AND v.Status IN "+finalStatuses;
                    }
                    else if(status=="pending") sql+=" AND v.Status NOT IN "+finalStatuses;
                    else if(status=="solved") sql+=" AND v.Status IN "+finalStatuses;
    ...
    void HandlePost(HttpContext c) {
        ...
        // Optional requester email (mobile parity with desktop SetTicketContactEmailAsync).
        // Validated only when supplied so older app versions keep working.
        string contactEmail=body.ContainsKey("contactEmail")&&body["contactEmail"]!=null?body["contactEmail"].ToString().Trim():"";
        if(!string.IsNullOrWhiteSpace(contactEmail) && !IsValidContactEmail(contactEmail)){Err(c,400,"contactEmail must be a valid email address");return;}
        ...
        try {
            using(var con=new SqlConnection(Cs())){
                ...
                using(var cmd=new SqlCommand("dbo.sp_Call_CreateTicket",con)){
                    ...
                    using(var r=cmd.ExecuteReader()){
                        if(r.Read()){
                            // sp_Call_CreateTicket returns only TicketId, TicketCode,
                            // Status, Priority, CreatedAt. Display fields (Company,
                            // Issue, Department, Branch, CallerName, IssueType) are
                            // echoed from the validated request, mirroring
                            // HelpItRepository which never reads them from this
                            // result set. Reading them here throws
                            // IndexOutOfRangeException ("Company") and masks a
                            // committed insert as HTTP 500, causing duplicate
                            ...
                            if(newTicketId>0)
                            {
                                // Store the requester email post-create: sp_Call_CreateTicket has no
                                // @ContactEmail parameter, so desktop does the same direct UPDATE.
                                // Separate connection because the proc reader above is still open (no MARS).
                                // Skipped silently when the migration column is absent, matching the
                                // readers elsewhere in this file. Runs before the emails so the
                                // NewTicket/Assignment mails resolve this recipient.
                                if(!string.IsNullOrWhiteSpace(contactEmail)){
                                    try{
                                        using(var eCon=new SqlConnection(Cs())){
                                            eCon.Open();
                                            if(ColumnExists(eCon,"dbo.CallTicket","ContactEmail")){
                                                using(var eCmd=new SqlCommand("UPDATE dbo.CallTicket SET ContactEmail=@E WHERE TicketId=@Id",eCon)){
                                                    eCmd.Parameters.Add("@E",SqlDbType.NVarChar,255).Value=contactEmail;
                                                    eCmd.Parameters.Add("@Id",SqlDbType.Int).Value=newTicketId;
                                ...
                                if (assignedToEmpId.HasValue && assignedToEmpId.Value > 0)
                                    ItcmEmailHelper.TrySendAssignmentEmailAsync(Cs(), newTicketId, createdByUserId);
                            }
                        } else {
...
SELECT CASE WHEN EXISTS(
    ...
    bool ColumnExists(SqlConnection con,string table,string column) {
        using(var cmd=new SqlCommand(@"
SELECT CASE WHEN EXISTS(
    SELECT 1 FROM sys.columns c
    INNER JOIN sys.objects o ON o.object_id=c.object_id
    INNER JOIN sys.schemas s ON s.schema_id=o.schema_id
    WHERE s.name='dbo' AND o.name=@Table AND c.name=@Column
) THEN 1 ELSE 0 END",con)){
            cmd.Parameters.Add("@Table",SqlDbType.NVarChar).Value=table.StartsWith("dbo.")?table.Substring(4):table;
            cmd.Parameters.Add("@Column",SqlDbType.NVarChar).Value=column;
            return Convert.ToInt32(cmd.ExecuteScalar())==1;
        }
    }

    // Same MailAddress round-trip rule as desktop TryNormalizeTicketContactEmail.
    bool IsValidContactEmail(string v) {
        try{
            var a=new MailAddress((v??string.Empty).Trim());
            return string.Equals(a.Address,(v??string.Empty).Trim(),StringComparison.OrdinalIgnoreCase);
...
WHERE ee.EmpId = @EmpId AND ee.IsPrimary = 1 AND ee.IsActive = 1 AND ea.IsActive = 1
ORDER BY ee.EmployeeEmailId;", con))
    ...
    private static void AddDistinct(List<string> target, IEnumerable<string> values)
    {
        foreach (var value in values ?? Enumerable.Empty<string>())
            if (!string.IsNullOrWhiteSpace(value) && !target.Contains(value, StringComparer.OrdinalIgnoreCase)) target.Add(value);
    }
