...
SELECT
    ...
    -- 🔹 Asset Info: first PartNumber/SerialNumber from active renewals for this set (correlated subquery, no fan-out)
...
CROSS APPLY (
    -- Effective end date: walk one level forward to the latest direct child renewal.
    -- Falls back to own EndDate when no child renewal exists.
    SELECT
        ISNULL(
