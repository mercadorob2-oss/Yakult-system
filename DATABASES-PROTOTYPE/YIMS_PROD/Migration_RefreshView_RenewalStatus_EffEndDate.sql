-- Migration: Refresh vw_RenewalStatus to use effective end date from renewal chain
-- Purpose: DaysUntilExpiry and ExpiryStatus now reflect the LATEST renewal set's EndDate
--          instead of the original (possibly expired) set's EndDate.
...
SELECT
    ...
    -- EffEndDate: if this set has a direct child renewal, use that child's EndDate.
    -- This means a "Fully Renewed" set shows its renewal's remaining days, not the expired original.
    CASE
...
CROSS APPLY (
    -- Effective end date: latest direct child's EndDate if renewed, else own EndDate
    SELECT ISNULL(
...
PRINT 'vw_RenewalStatus refreshed — DaysUntilExpiry now uses effective end date from renewal chain.';
