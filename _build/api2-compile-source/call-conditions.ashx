            ...
            using (var con = new SqlConnection(Cs()))
            {
                ...
                using (var chk = new SqlCommand("SELECT CASE WHEN OBJECT_ID('dbo.Condition','U') IS NOT NULL THEN 1 ELSE 0 END", con))
                {
                    sql = Convert.ToInt32(chk.ExecuteScalar()) == 1
                        ? "SELECT ConditionId, Name AS ConditionName FROM dbo.Condition ORDER BY Name"
                        : "SELECT ConditionId, Name AS ConditionName FROM dbo.ItemCondition ORDER BY Name";
                }
